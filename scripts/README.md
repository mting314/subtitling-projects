...
| `autosplit_long_lines.py` | Batch-fix all 3-row lines: drives `detect_long_lines.py` + `split_subtitle_line.py`, picking a safe clause boundary near each midpoint. **Guards against bad breaks** — never splits before `to` (infinitive), only uses `and`/`but`/`so` with a preceding comma (not `both X and Y`), prefers sentence ends. Defers lines with no safe boundary to a REWORD list, and audits the result for stranded conjunctions. `--apply` to write (default dry-run). Needs `uv run --with pillow --with numpy` + ffmpeg. | QC / layout |
