...
def transcript_to_json(
    transcript, time_offset: float = 0.0, chunk_duration: float = 0.0
) -> list[dict]:
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if duration <= MAX_WORD_TIMESTAMP_MINUTES * 60:
            ...
            print("  -> Under limit, no splitting needed")
            ...
            if input_gcs_uri is None:
                ...
                input_gcs_uri = (
                    f"gs://{args.gcs_bucket}/tmp/transcribe_input/{filename}"
                )
            ...
            try:
                ...
                print(
                    f"  API returned {num_results} result(s), {num_words} total words"
                )

                raw_data = transcript_to_json(
                    transcript, time_offset=trim_offset, chunk_duration=duration
                )
                all_raw_results.extend(raw_data)
                chunk_path = transcripts_dir / "transcript.json"
                chunk_path.write_text(
                    json.dumps({"results": raw_data}, indent=2, ensure_ascii=False),
                    encoding="utf-8",
                )
                print(f"  Saved to {chunk_path}")
            finally:
                if uploaded_input:
                    print("  Cleaning up temporary GCS file...")
                    try:
            ...
            print("  -> Exceeds limit, splitting required")
            ...
            for idx, (cp, cs) in enumerate(chunks):
                ...
                print(
                    f"    chunk {idx}: {cs:.0f}s - {cs + chunk_dur:.0f}s "
            ...
            print("\n  Transcribing chunks...")
            try:
                for i, ((chunk_path, chunk_start), chunk_uri) in enumerate(
                    zip(chunks, chunk_uris)
                ):
                    chunk_dur = get_audio_duration(chunk_path)
                    print(f"\n  --- Chunk {i + 1}/{len(chunks)} ---")
                    print(
                        f"  Time range: {seconds_to_timestamp(chunk_start)} - "
                    ...
                    raw_data = transcript_to_json(
                        transcript,
                        time_offset=chunk_start + trim_offset,
